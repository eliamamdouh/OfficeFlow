  ...
  Object.keys(schedule).forEach((month) => {
    ...
    Object.keys(monthSchedule).forEach((week) => {
      monthSchedule[week].forEach((day) => {
        console.log(
          `Checking date ${day.day} in week ${week} of month ${month}`
        ); // Log the current day being checked
        if (day.day === date) {
...
const submitRequest = async (req, res) => {
  try {
    ...
    if (!newDate || !dayToChange || !reason) {
      return res
        .status(400)
        .json("New date, day to change, and reason are required");
    }
    ...
    if (newDate === dayToChange) {
      return res
        .status(400)
        .json("The new date and the day to change cannot be the same");
    }
    ...
    if (locationForDayToChange === locationForNewDate) {
      ...
      return res
        .status(400)
        .json(
          "The selected days must be from different locations (one office, one home)"
        );
    }
  ...
  try {
    ...
    if (newDate === dayToChange) {
      return res
        .status(400)
        .json({
          message: "The new date and the day to change cannot be the same",
        });
    }
    ...
    if (!dayToChangeFound || !newDateFound) {
      return res
        .status(404)
        .json({ message: "One or both dates not found in the schedule" });
    }
